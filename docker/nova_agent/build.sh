...
OLD_PWD=$(pwd)
[ -d "build" ] || mkdir build
cd ../../chat-ui && [ -d "dist" ] || mkdir dist && reflex export --env prod --zip --zip-dest-dir ./dist && cd ${OLD_PWD} && mv ../../chat-ui/dist/* ./build/
...
tar -czvf server.tar.gz config.yaml *.so prompts backend.zip frontend.zip
rm -rf config.yaml *.build *.pyi ${module}.so prompts backend.zip frontend.zip
