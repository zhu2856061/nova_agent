...
_INTRODUCTION = "Hi! I'm **Nova Agent ThemeSlicer**, a helpful assistant."
MAX_TOTAL_CHARS = 150000  # 总字符保护限制
_DEFAULT_NAME = "Nova"


class AgentThemeSlicerState(State):
    unique_id = "Agent - ThemeSlicer"
    _default_introduction = _INTRODUCTION
    _default_name = _DEFAULT_NAME

    params_fields: list[Parameters] = [
        Parameters(
            mkey="theme_slicer_model",
            mtype="select",
            mvalue="basic",
            mvaluetype="str",
    ...
    async def process_question(self, form_data: dict[str, Any]):
        ...
        _url = STREAM_AGENT_THEMESLICER_BACKEND_URL
        ...
        async for item in get_agent_api(
            _url,
            self.current_chat,
            {"theme_slicer_messages": messages},
            config,
            {"result": question},
        ):  # type: ignore
