...
def history_action_modal(State, trigger) -> rx.Component:
    """侧边栏组件，用于显示所有项列表"""
    # 返回一个抽屉根组件
    return rx.drawer.root(
        ...
        rx.drawer.portal(  # 抽屉的门户容器，用于将抽屉渲染到DOM的其他位置
            rx.drawer.content(  # 抽屉的内容区域
                ...
                rx.vstack(
                    rx.heading("Actions", color=rx.color("mauve", 11)),  # 侧边栏标题
                    rx.divider(),  # 分隔线
...
def new_action_modal(State, trigger) -> rx.Component:
    """用于创建新聊天的模态框组件"""
