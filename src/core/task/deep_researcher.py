...
from core.tools import deep_researcher_tool, markdown_to_html_tool
from core.utils import get_today_str, set_color
...
async def clarify_with_user(
    ...
    def _assemble_prompt(messages):
        ...
        return [
            HumanMessage(content=apply_system_prompt_template("clarify_with_user", tmp))
        ]
    ...
    if not runtime.context.get("allow_clarification", True):
...
async def researcher(
    ...
    logger.info(
        set_color(
            f"trace_id={_trace_id} | node=researcher | message={response}",
            "pink",
        )
    )
...
async def researcher_tools(
    ...
    _trace_id = runtime.context.get("trace_id", "default")
    most_recent_message = state.get("researcher_messages", [])[-1]
    # Early Exit Criteria: No tool calls (or native web search calls)were made by the researcher
    if not most_recent_message.tool_calls:  # type: ignore
        return Command(
    ...
    tool_calls = most_recent_message.tool_calls  # type: ignore
    coros = []
    ...
    if state.get("tool_call_iterations", 0) >= runtime.context.get(
        "max_react_tool_calls", 5
    ) or any(
        tool_call["name"] == "ResearchComplete"
        for tool_call in most_recent_message.tool_calls  # type: ignore
    ):
        return Command(
            goto="compress_research",
            update={
...
# 转换问题
async def compress_research(state: ResearcherState, runtime: Runtime[Context]):
    # 变量
    ...
    # 提示词
    def _assemble_prompt(messages):
        tmp = {"date": get_today_str()}
        messages[0] = SystemMessage(
            content=apply_system_prompt_template("compress_research_system", tmp)
        )
        messages.append(
            HumanMessage(
    ...
    return {
        ...
        "raw_notes": [
            "\n".join(
                [
                    str(m.content)
                    for m in filter_messages(_messages, include_types=["tool", "ai"])
                ]
            )
        ],
    }
...
async def final_report_generation(state: AgentState, runtime: Runtime[Context]):
...
researcher_builder = StateGraph(
    ResearcherState, output_schema=ResearcherOutputState, context_schema=Context
)
...
graph_builder = StateGraph(
    AgentState, input_schema=AgentInputState, context_schema=Context
)
