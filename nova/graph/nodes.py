...
def coordinator_node(
    ...
    logger.info(f">>>>>>> 【{_task}】 ->Coordinator<- Activate. ")
    ...
    try:
        ...
        logger.info(f">>>>>>> 【{_task}】 ->Coordinator<- Exit. delegating to {goto}")
        if goto == "__end__":
            return Command(
                goto=goto,
                update={
        ...
        _tmp = f">>>>>>> 【{_task}】 ->Coordinator<- Exit. excute failed: {e}"
        logger.error(_tmp)
        return Command(
            goto="__end__",
...
def planner_node(
    ...
    configurable = Configuration.from_runnable_config(config)
    _task = configurable.task_id
    logger.info(f">>>>>>> 【{_task}】 ->Planner<- Activate. ")
    ...
    def _output_handle(response):
        ...
        steps = _response.get("steps")
        thought = _response.get("thought")
        title = _response.get("title")
        if steps is None or thought is None or title is None:
            logger.error(">>>>>>> Planner response not have steps field")
            return None
    ...
    try:
        ...
        _llm_response = _execute_llm(messages)
        response = _output_handle(_llm_response)
        if response is None:
            goto = "__end__"
        logger.info(f">>>>>>> 【{_task}】 ->Planner<- Exit. delegating to {goto}")
        return Command(
            goto=goto,
            update={"answer": _llm_response},
        )

    except Exception as e:
        _tmp = f">>>>>>> 【{_task}】 ->Planner<- Exit. excute failed: {e}"
        logger.error(_tmp)
        return Command(
            goto="__end__",
...
def supervisor_node(
    ...
    _task = configurable.task_id
    _task_dir = CONF["SYSTEM"]["TASK_DIR"] + "/" + _task
    logger.info(f">>>>>>> 【{_task}】 ->Supervisor<- Activate. ")
    ...
    try:
        ...
        if _answer.name == "planner" and len(_plan.get("steps", [])) > 0:
            ...
            logger.info(
                f"<<<<<<<< 【{_task}】 ->Supervisor<- Exit. delegating to: {goto}"
            )
        ...
        _tmp = f">>>>>>> 【{_task}】 ->Supervisor<- Exit. excute failed: {e}"
        logger.error(_tmp)
        return Command(
            goto="__end__",
...
def researcher_node(
    ...
    _task = configurable.task_id
    _task_dir = CONF["SYSTEM"]["TASK_DIR"] + "/" + _task
    logger.info(f">>>>>>> 【{_task}】 ->Researcher<- Activate. ")
    ...
    try:
        ...
        logger.info(f"<<<<<<< 【{_task}】 ->Researcher<- Exit.")
        return Command(
        ...
        _tmp = f">>>>>>> 【{_task}】 ->Researcher<- Exit. excute failed: {e}"
        logger.error(_tmp)
        return Command(
            update={
                "success": False,
...
def reporter_node(
    ...
    try:
        ...
        logger.info(f"<<<<<<< 【{_task}】 ->Reporter<- Exit.")
        return Command(
            update={
        ...
        _tmp = f">>>>>>> 【{_task}】 ->Reporter<- Exit. excute failed: {e}"
        logger.error(_tmp)
        return Command(
            update={
                "success": False,
