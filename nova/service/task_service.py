...
async def stream_deepresearcher_service(request: TaskRequest):
    """LLM Server"""
    if not request:
        raise HTTPException(status_code=400, detail="Input instances cannot be empty")
...
async def ainovel_service(request: TaskRequest):
    ...
    try:
        ...
        result = await ainovel.ainvoke(state, context=context)  # type: ignore
        content = result.get("messages")[-1].content  # type: ignore
        return TaskResponse(
            code=0,
            messages={"role": "assistant", "content": content},  # type: ignore
...
@task_router.post("/stream_ainovel", response_model=TaskResponse)
async def stream_ainovel_service(request: TaskRequest):
    """LLM Server"""
    if not request:
        raise HTTPException(status_code=400, detail="Input instances cannot be empty")
    ...
    try:
        ...
        async def async_service(trace_id, inputs, context) -> AsyncGenerator:
            ...
            try:
                session = aiohttp.ClientSession()  # 创建会话
                async for event in ainovel.astream_events(
                    inputs, context=context, version="v2"
                ):
                    data = handle_event(trace_id, event)
                    if data:
